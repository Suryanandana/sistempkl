<?php
error_reporting(0);
if($_GET['token'] != $this->session->userdata('string')) :
?>
    <script>
    ...
    <div id="auth">
        <div class="row h-100" style="overflow-x: hidden;">
            <div class="col-lg-5 col-12 order-1">
                <div id="auth-left">
                    <h1 class="auth-title text-center" style="font-size: 60px;">Ubah Password</h1>
                    <form name="formlogin" id="formlogin" method="post"
                        action="<?php echo base_url('cauthentication/prosesUbahPassword'); ?>" class="mt-4">
                        <?php if ($this->session->has_userdata('login_gagal')): ?>
                            <div class="alert alert-danger alert-dismissible fade show position-relative" role="alert">
                                ...
                                <button type="button" class="btn-close position-absolute"
                                    style="top:50%; transform: translateY(-50%);" data-bs-dismiss="alert"
                        ...
                        <div class="form-group position-relative has-icon-left mb-4">
                            ...
                            <input type="hidden" value="<?= $this->session->userdata('username_ubah') ?>" name="username">
                            <div class="form-control-icon">
                                <i class="bi bi-shield-lock"></i>
                            </div>
                        </div>
                        <button class="btn btn-primary btn-block btn-lg shadow-lg mt-2"
                            onclick="proseslogin()">Simpan</button>
                    </form>
